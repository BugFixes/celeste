...
#		--item '{"agent_id":{"S":"${BUGFIXES_GITHUB_AGENT_ID}"},"comms_details":{"M":{"channel":{"S":"${DISCORD_TEST_CHANNEL}"}}},"id":{"S":"${BUGFIXES_GITHUB_AGENT_ID}"},"system":{"S":"discord"}}' 1> /dev/null
	aws dynamodb put-item \
      		...
      		--region us-east-1 \
      		--table-name comms \
      		--item '{"agent_id":{"S":"${BUGFIXES_GITHUB_AGENT_ID}"},"comms_details":{"M":{"channel":{"S":"${SLACK_TEST_CHANNEL}"},"token":{"S":"${SLACK_BOT_TOKEN}"}}},"id":{"S":"${BUGFIXES_GITHUB_AGENT_ID}"},"system":{"S":"slack"}}' 1> /dev/null
	#aws dynamodb put-item \
#  		--endpoint http://localhost.localstack.cloud:4566 \
#  		--region us-east-1 \
#  		--table-name comms \
