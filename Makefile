...
injectData: # Inject Agent
	...
	aws dynamodb put-item \
		...
		--item '{"access_token":{"S":"${GITHUB_ACCESS_TOKEN}"},"id":{"S":"github_token"},"system":{"S":"github"},"agent_id":{"S":"bob"},"ticketing_details":{"M":{"owner":{"S":"bugfixes"},"repo":{"S":"celeste"},"installation_id":{"S":"${GITHUB_INSTALL_ID}"}}}}' 1> /dev/null
	aws dynamodb put-item \
		--endpoint http://localhost.localstack.cloud:4566 \
		--region us-east-1 \
		--table-name ticketing \
		--item '{"access_token":{"S":"${JIRA_API_TOKEN}"},"id":{"S":"bob"},"system":{"S":"jira"},"agent_id":{"S":"${BUGFIXES_AGENT_ID}"},"ticketing_details":{"M":{"project":{"S":"automated-bugs"},"username":{"S":"${JIRA_USERNAME}"}}}}' 1>/dev/null
	aws dynamodb put-item \
		--endpoint http://localhost.localstack.cloud:4566 \
		--region us-east-1 \
		--table-name agents \
		--item '{"id":{"S":"${BUGFIXES_AGENT_ID}"},"name":{"S":"bob"},"account_record":{"M":{"id":{"S":"${BUGFIXES_ACCOUNT_ID}"},"name":{"S":"bob"},"email":{"S":"${BUGFIXES_ACCOUNT_EMAIL}"},"level":{"S":"0"},"account_credentials":{"M":{"key":{"S":"${BUGFIXES_AGENT_KEY}"},"secret":{"S":"${BUGFIXES_AGENT_SECRET}"}}}}},"agent_credentials":{"M":{"key":{"S":"${BUGFIXES_AGENT_KEY}"},"secret":{"S":"${BUGFIXES_AGENT_SECRET}"}}}}' 1> /dev/null
	aws dynamodb put-item \
		--endpoint http://localhost.localstack.cloud:4566 \
		--region us-east-1 \
		--table-name accounts \
		--item '{"id":{"S":"${BUGFIXES_ACCOUNT_ID}"},"name":{"S":"bob"},"email":{"S":"${BUGFIXES_ACCOUNT_EMAIL}"},"level":{"S":"0"},"account_credentials":{"M":{"key":{"S":"${BUGFIXES_ACCOUNT_KEY}"},"secret":{"S":"${BUGFIXES_ACCOUNT_SECREET}"}}}}' 1> /dev/null
